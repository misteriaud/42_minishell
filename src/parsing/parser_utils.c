...
/*   Updated: 2022/05/18 17:32:00 by mriaud           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_err	concat_token(t_ctx *ctx, t_token *token, t_state *state, t_str tmp)
{
	...
	dest = (t_str){NULL, 0};
	if (token->value.str && token->value.str[0] == '$'
		&& token->value.len == 1 && state->prev & (IN_DQ | IN_SQ))
		token->value = (t_str){0, 0};
	if (!(state->prev & IN_SQ) && token->type != HEREDOC
		&& drop_variables(ctx, &tmp))
		return (MEMORY_ERROR);
	if (!token->value.str)
		token->value = tmp;
